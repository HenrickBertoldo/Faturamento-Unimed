...
def detectar_codigos_suspeitos(dfs):
    """Verifica, nas tabelas de regras recém-carregadas do Sheets, códigos
    que provavelmente perderam zero(s) à esquerda porque a célula na
    planilha foi interpretada como número em vez de texto — o mesmo tipo de
    bug já visto de verdade em produção, tanto num código de item (aba
    'itens') quanto num código de plano (aba 'blindagem'). Esta função não
    corrige nada sozinha (padronizar_codigo_8_digitos e
    identificar_plano_pela_carteira já cuidam disso durante o
    processamento) — ela só avisa, apontando aba/coluna/valor, para quem
    mantém a planilha decidir se o código está mesmo errado ou se aquele
    tamanho menor é legítimo."""
    avisos = []

    def _tamanho_suspeito(valor, tamanho_esperado):
        v = limpar_numero(valor)
        return v.isdigit() and 0 < len(v) < tamanho_esperado

    # Tabelas cujos códigos, pela convenção da TISS, têm sempre 8 dígitos.
    tabelas_8_digitos = {
        'itens': ['Código Incorreto', 'Código Correto'],
        'unidades': ['Código do Item'],
        'anvisa': ['Código do Item'],
        'procedimentos': ['Código do Procedimento'],
    }
    for aba, colunas in tabelas_8_digitos.items():
        df = dfs.get(aba)
        ...
        for col in colunas:
            ...
            for valor in df[col].dropna().unique():
                if _tamanho_suspeito(valor, 8):
                    v = limpar_numero(valor)
                    avisos.append(
                        f"Aba '{aba}', coluna '{col}': código '{v}' tem só {len(v)} dígito(s) (códigos da TISS "
                        f"costumam ter 8). Se a célula não estiver formatada como Texto no Sheets, pode ter "
                        f"perdido zero(s) à esquerda — o app já compensa isso automaticamente (viraria "
                        f"'{v.zfill(8)}'), mas vale conferir a formatação da célula para não depender disso."
                    )

    # Aba 'blindagem': código de PLANO tem 4 dígitos (mesmo prefixo usado por
    # identificar_plano_pela_carteira); código de PROCEDIMENTO tem 8.
    df_blindagem = dfs.get('blindagem')
    if df_blindagem is not None and not df_blindagem.empty and 'Tipo' in df_blindagem.columns and 'Código' in df_blindagem.columns:
        for _, r in df_blindagem.iterrows():
            tipo = str(r.get('Tipo', '')).strip().upper()
            valor = r.get('Código', '')
            if tipo == 'PROCEDIMENTO' and _tamanho_suspeito(valor, 8):
                v = limpar_numero(valor)
                avisos.append(
                    f"Aba 'blindagem' (linha de PROCEDIMENTO): código '{v}' tem só {len(v)} dígito(s) "
                    f"(esperado 8). Confira a formatação da célula 'Código' (viraria '{v.zfill(8)}')."
                )
            elif tipo == 'PLANO' and _tamanho_suspeito(valor, 4):
                v = limpar_numero(valor)
                avisos.append(
                    f"Aba 'blindagem' (linha de PLANO): código '{v}' tem só {len(v)} dígito(s) "
                    f"(esperado 4). Confira a formatação da célula 'Código' (viraria '{v.zfill(4)}')."
                )

    # Evita listar o mesmo aviso repetido (ex.: mesmo código incorreto em
    # duas linhas de 'blindagem'), preservando a ordem de primeira ocorrência.
...
def validar_e_recalcular_xml_editado(texto_editado):
    ...
    # 🛡️ Nomes de tag da TISS são sempre só letras (ex.: numeroGuiaPrestador,
    # codigoProcedimento) — nunca têm dígito misturado no NOME da tag (só no
    # conteúdo dela). Um XML pode ser sintaticamente válido (ET.fromstring
    # aceita qualquer nome de tag) e mesmo assim ter uma tag corrompida por
    # uma edição acidental — por exemplo, um autocorretor de teclado (comum
    # em tablets no modo de tela dividida) trocando parte do texto no meio
    # de uma digitação direta no editor. Isso passaria batido no "parece
    # certo visualmente" e só apareceria ao baixar/copiar o arquivo. Esta
    # checagem pega exatamente esse padrão antes de deixar salvar.
    tags_corrompidas = sorted({
        tag_limpa(elem) for elem in root.iter() if any(c.isdigit() for c in tag_limpa(elem))
    })
    if tags_corrompidas:
        return None, (
            "Encontrei tag(s) com número misturado no nome, o que nunca é válido na TISS — "
            f"provavelmente uma edição acidental: {', '.join(tags_corrompidas)}. Confira essa parte do "
            "texto (Ctrl+F pelo nome da tag) antes de salvar; o download foi bloqueado para não gerar "
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    # este arquivo como módulo — por exemplo, para rodar os testes
    # automatizados em test_app.py — já subiria o servidor de verdade.
    ui.run(
